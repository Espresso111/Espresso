...
def cal_op(micro_batches, num_stages, stage_id, useNew = False):
    _INSTRUCTION_MAP = {
...
if __name__ == "__main__":
    fwd = [0.017221,0.131497,0.142332,0.103559]
    # fwd = [72.79, 521.93,561.38,406.61]
    print(solve(fwd, 4))
    ...
    #     tmp = cal_op(4,pp_size,stage_id)
    #     ans.append(tmp)
    # # tmp = ["F","F","F","F","B","B","B","B"]
    # # ans = [tmp for x in range(pp_size)]
    # std = []
    ...
    #         # print(f"step_cmds = {step_cmds}")
    #         b.extend(step_cmds)
    #         std.extend(step_cmds)
    #     print(b)
    # print("!!!!")
    # tmp = ["F","F","F","F","B","B","B","B"]
    # ans = [tmp for x in range(pp_size-1)]
